...
namespace Com
{
	namespace Import
	{
		TypeDescription::TypeDescription(const std::string& libraryName, Pointer<ITypeInfo2> typeInfo, const TYPEDESC& description)
			: libraryName(libraryName), typeInfo(typeInfo), description(description)
		{
		}

		Type TypeDescription::ToType() const
		{
			return DetermineType(description, 0);
		}

		Type TypeDescription::DetermineType(const TYPEDESC& typeDescription, int indirection) const
		{
		...
		Type TypeDescription::ToArrayType(const ARRAYDESC& arrayDescription, int indirection) const
		{
			if (indirection != 0)
		...
		TypeEnum TypeDescription::ToArrayTypeEnum(VARTYPE vt)
		{
			switch (vt)
			{
		...
		TypeEnum TypeDescription::ToBasicTypeEnum(VARTYPE vt)
		{
		...
		Type TypeDescription::ToUserDefinedType(HREFTYPE handle, int indirection) const
		{
		...
		std::string TypeDescription::GetUserDefinedTypeName(Pointer<ITypeInfo> customType) const
		{
		...
		TYPEKIND TypeDescription::GetTypeKind(Pointer<ITypeInfo> customType)
		{
		...
		std::string TypeDescription::GetDefaultInterfaceName(Pointer<ITypeInfo> customType) const
		{
			return TypeInfo{ customType }.GetInterfaceName(0);
		}
	}
}
